# Troubleshooting
While Notely is actively maintained and stable, encountering bugs is possible.
...
## Broken Note Crashes Notely

Certain problems, such as rendering a note with a faulty script, can cause Notely to crash. If Notely attempts to reload the problematic note upon restart, it will continue to crash.
...
If a custom script causes Notely to crash, and it is set as a startup script or in an active [custom widget](Scripting/Frontend%20Basics/Custom%20Widgets.md), start Notely in "safe mode" to prevent any custom scripts from executing:

```
TRILIUM_SAFE_MODE=true ./trilium
```

Depending on your Notely distribution, you may have pre-made scripts available: `trilium-safe-mode.bat` and `trilium-safe-mode.sh`.

Once Notely starts, locate and fix or delete the problematic note.

## Sync and Consistency Checks

Notely periodically verifies the logical consistency of the database (e.g., ensuring every note has a parent). If inconsistencies are detected, you will be notified via the UI.

In such cases, file a bug report and attach an [anonymized database](Troubleshooting/Anonymized%20Database.md) if necessary.

## Restoring Backup

Notely makes regular automatic backups. If issues become severe, you can [restore from a backup](Installation%20%26%20Setup/Backup.md).
