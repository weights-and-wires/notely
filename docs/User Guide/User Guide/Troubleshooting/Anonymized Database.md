...
In certain scenarios, understanding the structure of a database is crucial for troubleshooting issues. However, sharing your actual [database](../Advanced%20Usage/Database.md) file with personal notes is not advisable. To address this, Notely offers a feature to anonymize the database. This feature can be accessed via Menu -> Options -> Advanced tab.
...
If your [database](../Advanced%20Usage/Database.md) is corrupted to the point where Notely cannot start, the anonymization process can still be executed via the command line:

```
node src/anonymize.js
```

Run this command from the directory containing Notely's source files, typically found in the `resources/app` directory for desktop builds.