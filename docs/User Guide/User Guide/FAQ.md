# FAQ
## Inspiration for the name “Notely”
...
> 
> _– Zadam (original Notely maintainer)_

> [!NOTE]
> Despite the fact that the Trillium flower has two “l"s, the Notely application only has one.

## macOS support

Originally, Notely considered the macOS build unsupported. Notely commits to make the experience on macOS as good as possible.
...
The original Notely application did not support multiple languages. Since we believe that internationalisation is a core part of an application, we have added support for it.
...
*   it's a huge feature, or rather a Pandora's box of collaboration features like user management, permissions, conflict resolution, real-time editing of a note by multiple people etc. This would be a huge amount of work. Notely is project made mostly by one person in free time and that's unlikely to change in the future.
...
These general purpose sync apps are not suitable to sync database files which are open and being worked on by another application. The result is that they will corrupt the database file, resulting in data loss and this message in the Notely logs:

```
SqliteError: database disk image is malformed
```

The only supported way to sync Notely's data across the network is to use a [sync/web server](Installation%20%26%20Setup/Synchronization.md).

## Why database instead of flat files?

Notely stores notes in a [database](Advanced%20Usage/Database.md) which is an SQLite database. People often ask why doesn't Notely rather use flat files for note storage - it's fair question since flat files are easily interoperable, work with SCM/git etc.

Short answer is that file systems are simply not powerful enough for what we want to achieve with Notely. Using filesystem would mean fewer features with probably more problems.
...
*   filesystems make a distinction between directory and file while there's intentionally no such difference in Notely
*   files are stored in no particular order and user can't change this
*   Notely allows storing note [attributes](Advanced%20Usage/Attributes.md) which could be represented in extended user attributes but their support differs greatly among different filesystems / operating systems
*   Notely makes links / relations between different notes which can be quickly retrieved / navigated (e.g. for [note map](Advanced%20Usage/Note%20Map%20\(Link%20map%2C%20Tree%20map\).md)). There's no such support in file systems which means these would have to be stored in some kind of side-car files (mini-databases).
...
Notely places exact matches before fuzzy matches. When you search for "project", notes containing exactly "project" appear before notes with variations like "projects" or "projection", regardless of other scoring factors.
